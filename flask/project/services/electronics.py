...
def create_desktop(price, weight, brand, processor, ram_size, cpu_cores, hd_size, dimensions):
    try:
        if validate_price(price) \
                and validate_weight(weight) \
                and validate_ram_size(ram_size) \
                and validate_cpu_cores(cpu_cores) \
                and validate_hd_size(hd_size):
...
def create_laptop(price, weight, brand, display_size, processor, ram_size, cpu_cores, hd_size, battery_info, os,
                  touchscreen, camera):
    try:
        if validate_price(price) \
                and validate_weight(weight) \
                ...
                and validate_cpu_cores(cpu_cores) \
                and validate_hd_size(hd_size):
            laptop = Laptop(model=str(uuid4()), price=price, weight=weight, brand=brand, display_size=display_size,
                            processor=processor,
                            ram_size=ram_size, cpu_cores=cpu_cores, hd_size=hd_size, battery_info=battery_info, os=os,
                            touchscreen=touchscreen, camera=camera)
...
def create_tablet(price, weight, brand, display_size, dimensions, processor, ram_size, cpu_cores, hd_size, battery, os,
                  camera_info):
    try:
        if validate_price(price) \
                and validate_weight(weight) \
                ...
                and validate_cpu_cores(cpu_cores) \
                and validate_hd_size(hd_size):
            tablet = Tablet(model=str(uuid4()), price=price, weight=weight, brand=brand, display_size=display_size,
                            processor=processor,
                            ram_size=ram_size, cpu_cores=cpu_cores, hd_size=hd_size, battery=battery, os=os,
